...
Find the Raspberry Pi's IP address in your network settings (it will probably be `192.168.2.1`).  Open up your terminal or command prompt, and run `ssh root@[IP address]` to enter the root of your Pi or, To log into the Pi user use `ssh pi@[local IP address]` and to log into root, you can run `sudo -s`.Run.
```
